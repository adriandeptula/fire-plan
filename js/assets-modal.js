...
function editA(id) {
  ...
  // Point 2: hide type selector when editing
  const atypeRow = g("atype-row");
  ...
  onTC();
  // Point 1: only prefill fields relevant to type; clear irrelevant ones
...
function onTC() {
  ...
  // Point 1: hide units for manual/nier; show only relevant value inputs
  g("aurow").style.display = isManual || isNier ? "none" : "block";
  g("awynajem-row").style.display = isNierWynajem ? "block" : "none";
  g("amrow").style.display =
    isManual || isNierSprzedaz ? "block" : "none";
...
// Point 6: format units cleanly — no trailing zeros, no 'szt.'
function fmtUnits(u) {
...
// Point 5: normalize comma to dot for numeric inputs (defined in model params above)
async function saveAsset() {
  const op = g("aop")?.value || "buy";
  const t = g("atype").value,
    [tg, td] = t.split(":");
  const ticker =
    td === "OTHER" ? g("atick").value.trim().toUpperCase() : td;
  const isNier = tg === "nier-sprzedaz" || tg === "nier-wynajem";
  const isManual = tg === "manual";
  // Point 1: units only from units field, not carried over from other type
  ...
  // Ogranicz historię do 500 wpisów (najstarsze usuwane)
