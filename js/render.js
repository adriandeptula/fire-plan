// ── RENDER ALL ──
// Cache wyniku sim() żeby nie liczyć 5x przy każdym rA()
let _simCache = null;
...
let _rATimer = null;
function rA() {
  // Debounce — max 1 render co 80ms przy szybkim pisaniu
  clearTimeout(_rATimer);
  _rATimer = setTimeout(_rAImmediate, 80);
}
function _rAImmediate() {
  _simCacheKey = null; // Unieważnij cache przy każdym render
  uIkeNetDisplay();
  rDash();
  rPortfel();
  rBud();
...
function rDash() {
  ...
  const infPct = ((Math.pow(1 + inf, realYrsToFire) - 1) * 100).toFixed(
    1,
  );

  // Zobowiązania — widoczne tylko gdy > 0
  ...
  if (r) {
    ...
    if (fireBanner)
      fireBanner.style.display = fireAchieved ? "flex" : "none";
    ...
    sT2(
      ...
      yrsLeft <= 0
        ? `Rok ${Math.round(r.fy)}`
        : `Rok ${Math.round(r.fy)} · za ${yrsLeft} lat`,
    );
    sT2("ki", PLN(pf(S.inv)));
    // Od 60 — dziś jako główna
...
function rPortfel() {
  ...
  sT2("pike", PLN(gIKE()));
  sT2("ppoza", PLN(gPoza()));
  const nier = gNierSprzedaz(),
    wyn = getWynajemBrutto();
  let extra = "";
  if (nier > 0)
...
function rBud() {
  ...
  const inv = pf(S.inv),
    ...
    i2 = pf(S.i2) || 26019,
    ip = (pf(S.ip) || 100) / 100;
  const ikM = ((i1 + i2) / 12) * ip,
    pM = Math.max(0, inv - ikM);
  ...
  // Przeliczenie celu FIRE przy wpisywaniu wypłaty
  const wy = pf(S.wy),
    wiek = pf(S.wt) || 31,
    wf = pf(S.wf) || 50,
    inf = getINF();
  ...
  const infPct = ((Math.pow(1 + inf, latDoFIRE) - 1) * 100).toFixed(1);
  const nomEl = g("bwy-nom");
  if (nomEl && wy > 0) {
    nomEl.innerHTML = `<span>= ${PLN(wyNom)}/mies. w roku ${new Date().getFullYear() + latDoFIRE}</span> · inflacja +${infPct}%`;
  } else if (nomEl) {
    nomEl.innerHTML = "";
  }
}
...
<div class="cs inf-badge"><span>${PLN(r.G)} nominalnie · +${infPct}%</span></div>
    ...
    <div class="card"><div class="ct" style="background:var(--gr)"></div><div class="cl">Rok FIRE</div><div class="cv gr">${fY}</div><div class="cs">Za ${Math.round(r.yr)} lat · wiek ${fA} lat</div></div>
...
<div class="cs">${PLN(r.m60)}/mies. nominalnie</div>
  ...
  <div class="card"><div class="tl">
    ...
    ${r.cy ? `<div class="ti"><div class="td"></div><div class="ty">${r.cy} · CoastFIRE</div><div class="tt">Portfel pracuje sam</div><div class="tdc">Od tego roku portfel urośnie do celu bez żadnych nowych wpłat — wzrost procentowy pokrywa resztę drogi. Formuła: portfel ÷ (1+inflacja)^lat_do_celu.</div></div>` : ""}
